(function(){
  angular
    .module('buy')
    .controller('buyIceboxController',buyIceboxController)

    function buyIceboxController($scope,$cookies,timeNow,$window, $state,$stateParams,service,serviceAlert,serviceConfirm){
      var vm = this;
      ...
      function selectProductKind(productId,productName){
        ...
        return vm.promise.then(function(response) {
          if(response.res === '1'){
            ...
            vm.screen = vm.productInfo.屏幕类型;
            vm.simCard = vm.productInfo.全网通双卡和移动网络;
            vm.navigation = vm.productInfo.导航定位;
            vm.sensors = vm.productInfo.感应器;
            vm.playback = vm.productInfo.多媒体播放;
      ...
      function selectVersion(version,index){
        ...
        return vm.promise.then(function(response) {
          if(response.res === '1'){
            ...
            vm.mainBody = vm.product[0].productData.主体;
            vm.configuration = vm.product[0].productData.配置;
            vm.weightSize = vm.product[0].productData.重量与尺寸;
